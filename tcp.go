...
func handleTCPConn(server *TailscaleProxyServer, c1 net.Conn, c2 net.Conn) {
	...
	if c2 == nil {
		...
		if err != nil {
			ReportError(err, "tcp error")
			if err := c1.Close(); err != nil {
				ReportError(err, "error closing connection")
			}
	...
	cp := func(dst net.Conn, src net.Conn) {
		...
		case first <- struct{}{}:
			...
			if err := dst.Close(); err != nil {
				ReportError(err, "error closing destination connection")
			}
			if err := src.Close(); err != nil {
				ReportError(err, "error closing source connection")
			}
			slog.Info("disconnected", "remote_addr", c1.RemoteAddr())
		default:
		}
